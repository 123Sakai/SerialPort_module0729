...
void SerialPort_module0729::on_sendButton_clicked()
{
    ...
    qDebug() << "raw buffer:" << send_data;
    if (ui.ishex_send->isChecked() == false)
    {
        m_serialPort->send_ascii_data(send_data);
    }
    else if (ui.ishex_send->isChecked() == true)
    {
...
void SerialPort_module0729::recv_data(const QString &data)
{
    ...
    //recv_buffer = m_serialPort->get_data();
    if (ui.ishex_recv->isChecked())
    {
        ui.receive_text_window->append(currentTimeString + "\r\n" + data.toLocal8Bit().toHex().toUpper().constData());
    }else
    { 
        ui.receive_text_window->append(currentTimeString + "\r\n" + data.toLocal8Bit());
    }
